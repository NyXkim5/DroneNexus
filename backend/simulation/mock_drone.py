...
class MockDrone:
    ...
    def update(self, dt: float, leader: Optional["MockDrone"] = None) -> None:
        ...
        if self.drone_sim_state in ("IDLE", "LANDED"):
            self._update_grounded(s)
            return
        if self.drone_sim_state == "ARMED":
            self._update_armed(s)
            return
        if self.drone_sim_state == "TAKING_OFF":
            self._update_takeoff(s, dt)
            return
        if self.drone_sim_state == "LANDING":
            self._update_landing(s, dt)
            return
        if self.drone_sim_state == "EMERGENCY":
            self._update_emergency(s, dt)
            return
        if self.drone_sim_state == "MISSION":
            self._update_mission(s, dt)
            return
        if self.drone_sim_state == "GOTO":
            self._update_goto(s, dt)
            return

        # FLYING state
        self._update_flying(s, dt, leader)

    def _update_grounded(self, s: DroneState) -> None:
        s.ground_speed = 0
        s.vertical_speed = 0
        s.in_air = False
        s.armed = False
    ...
    def _update_armed(self, s: DroneState) -> None:
        s.ground_speed = 0
        s.vertical_speed = 0
        s.in_air = False
        s.armed = True
    ...
    def _update_takeoff(self, s: DroneState, dt: float) -> None:
    ...
    def _update_landing(self, s: DroneState, dt: float) -> None:
    ...
    def _update_emergency(self, s: DroneState, dt: float) -> None:
    ...
    def _fly_toward(self, s: DroneState, dt: float,
                    target_lat: float, target_lon: float,
                    target_alt: float) -> float:
        """Steer toward a lat/lon/alt target. Return distance in meters."""
    ...
    def _apply_battery_and_status(self, s: DroneState, dt: float) -> None:
        """Drain battery and set nominal flight status."""
    ...
    def _update_mission(self, s: DroneState, dt: float) -> None:
        ...
        dist = self._fly_toward(s, dt, target_lat, target_lon, target_alt)
        if dist < 3.0:
            logger.info(
        ...
        self._apply_battery_and_status(s, dt)

    def _update_goto(self, s: DroneState, dt: float) -> None:
    ...
    def _update_flying_position(self, s: DroneState, dt: float,
                                leader: Optional["MockDrone"],
                                t: float) -> None:
        """Update position for the FLYING state based on role."""
    ...
    def _update_flying_sensors(self, s: DroneState, dt: float, t: float) -> None:
        """Update altitude, attitude, speed, battery, sensors, and FPV data."""
    ...
    def _update_flying(self, s: DroneState, dt: float,
                       leader: Optional["MockDrone"]) -> None:
        """Full FLYING state update: position, sensors, and timing."""
        t = time.time()
        s.in_air = True
        s.armed = True
        self._update_flying_position(s, dt, leader, t)
        self._update_flying_sensors(s, dt, t)
        s.last_update = time.monotonic()
